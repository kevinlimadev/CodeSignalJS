...
There are four 3 × 3 squares in the input image, so there should
 ...
 (7 + 4 + 0 + 5 + 6 + 2 + 6 + 10 + 7) = 47 / 9 = 5.2222 = 5. The 
 other three integers are obtained the same way, then the surrounding
...
function solution(image) {
  ...
  for (let i = 0; i < row - 2; i++) {
    ...
    for (let j = 0; j < col - 2; j++) {
      ...
      rowBlur.push(Math.floor(sum / 9));
    }
    blurredImage.push(rowBlur);
  }
